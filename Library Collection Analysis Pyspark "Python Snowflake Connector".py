...
# MAGIC =====================================================
# MAGIC 
# MAGIC This is a demonstration of how you can use Azure Databricks to categorise text documents into different topics from your Snowflake database tables.  
...
# MAGIC 
# MAGIC I will show you how with Azure Databricks you can:
# MAGIC - Explore data
# MAGIC - Visualise data
# MAGIC - Build a recommendation engine
# MAGIC - Pull and push data from and into your Snowflake database
# MAGIC 
# MAGIC All powered by a 3 node Azure Spark cluster!
# MAGIC 
# MAGIC Note:  
# MAGIC *Cluster specs: Azure Standard_DS3_v2 with 2 worker nodes and autoscaling enabled.  
# MAGIC Total run time for different sample sizes: 55mins for 1m.*
# MAGIC 
# MAGIC <img src="https://upload.wikimedia.org/wikipedia/commons/4/4d/Seattle_Public_Library.jpg" width="1000"></img>
# MAGIC 
...
# MAGIC ----------------------------------
# MAGIC 
# MAGIC In order to extract the data from the Snowflake database, you must perform one of these steps:
# MAGIC - Attach the spark-snowflake connector package to your spark cluster.
# MAGIC - Attach the snowflake python connector and snowflake.sqlalchemy.
# MAGIC 
# MAGIC NB: you will need to follow one of these steps before you are able to execute read/write commands with the Snowflake database.
...
sf_credentials = {
  ...
  "password": password,
  "role": "DATABRICKS",
  "database": "DATABRICKS_DEMO",
  "schema": "SEATTLE_LIBRARY",
...
features = pd.read_sql_query(
  sql=(
    "select title, bibnum, subjects from library_collection_inventory where reportdate in (\
    '2017-09-01T00:00:00',\
    '2017-10-01T00:00:00',\
    '2017-11-01T00:00:00',\
    '2017-12-01T00:00:00',\
...
class TokenizeAndStemer(TransformerMixin, BaseEstimator):
  ...
  * Stemming: The process of breaking a word down into its root.
  ...
  def transform(self, X):
    ...
    allwords_stemmed = [self.tokenize_and_stem(i) for i in X[self.text_column].tolist()]
    totalvocab_stemmed.extend(allwords_stemmed)
    totalvocab_stemmed = list(more_itertools.collapse(totalvocab_stemmed))
    totalvocab_tokenized = []
    # for each item in 'synopses', tokenize/stem
    allwords_tokenized = [self.tokenize_only(i) for i in X[self.text_column].tolist()]
...
finish = time.time()
print("Time to complete:", round((finish-start)/60, 2))
